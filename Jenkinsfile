...
pipeline {
    ...
    post {
        ...
        success {
            ...
            // Publish HTML coverage report for viewing directly in Jenkins.
            // 'htmlcov' directory and 'index.html' are generated by 'coverage html'.
            publishHTML([
                allowMissing: false,          
                alwaysLinkToLastBuild: true,  
                keepAll: true,                
            ...
            // Include test results for later analysis
            archiveArtifacts artifacts: 'test-results.xml,coverage.xml,htmlcov/**/*', allowEmptyArchive: true
        }

        // Else failure - run this script
        failure {
            ...
            publishHTML([
                ...
                reportDir: 'htmlcov',
                reportFiles: 'index.html',
                reportName: 'Coverage Report (Failed Build)',
                reportTitles: 'Test Coverage'
            ])
            archiveArtifacts artifacts: 'test-results.xml,coverage.xml,htmlcov/**/*', allowEmptyArchive: true
        }

        // Unstable script runs if the pipeline completes but some tests/checks are unstable
        unstable {
            ...
            publishHTML([
                ...
                reportDir: 'htmlcov',
                reportFiles: 'index.html',
                reportName: 'Coverage Report (Unstable Build)',
                reportTitles: 'Test Coverage'
            ])
            archiveArtifacts artifacts: 'test-results.xml,coverage.xml,htmlcov/**/*', allowEmptyArchive: true
        }
